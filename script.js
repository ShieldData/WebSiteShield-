...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ═══ BREACH STATISTICS COUNTERS ═══
  function formatBreachVal(el, current, target) {
  ...
  function countBreachVal(el, duration = 1800) {
    ...
    const step = ts => {
      ...
      const p = Math.min((ts - start) / duration, 1);
      const ease = 1 - Math.pow(1 - p, 3);
      formatBreachVal(el, ease * target, target);
      if (p < 1) requestAnimationFrame(step);
    };
    requestAnimationFrame(step);
  }

  const breachObs = new IntersectionObserver(entries => {
    entries.forEach(e => {
      if (e.isIntersecting) {
        document.querySelectorAll('[id^="bc"]').forEach(el => countBreachVal(el));
        document.querySelectorAll('.bc-bar-fill').forEach(bar => bar.classList.add('animate'));
        breachObs.disconnect();
      }
    });
  }, { threshold: 0.2 });
  const breachSec = document.getElementById('threats');
  if (breachSec) breachObs.observe(breachSec);
